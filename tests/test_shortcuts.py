...
def test_create_shortcut():
    ...
    assert f'{DEFAULT_TARGET_FOLDER}\\{test_rom.platform.name}\\{test_rom.filename}->{test_rom.source}' in test[LoggerFlag.SUCCESS]
    ...
    test = create_shortcut(test_rom)
    assert f'{DEFAULT_TARGET_FOLDER}\\{test_rom.platform.name}\\{test_rom.filename}.lnk->{test_rom.source}' in test[LoggerFlag.SUCCESS]
...
def test_dry_run():
    ...
    assert f'{DEFAULT_TARGET_FOLDER}\\{test_rom.platform.name}\\{test_rom.filename}->{test_rom.source}' in test[LoggerFlag.DRY_RUN]


def test_dry_blacklist():
    test_rom = SteamLibraryIndexItem({f'{TEST_FILES_PATH}\\steam': '228980'})
    test = create_shortcut(test_rom, dry_run=True)
    assert LoggerFlag.BLACKLIST in test.keys()
